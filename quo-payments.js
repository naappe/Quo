/* Quo v41 - atomic invoice payments with automatic immutable receipts. */
(function(){
  const EPS=0.005;
  const maleToday=()=>{
  ...
  function ensurePaymentModal(){
    ...
    wrap.innerHTML=`<div class="modal-card quo-payment-card">
      ...
      <div class="form-grid two quo-payment-fields"><div class="field"><label>Amount Received</label><input id="quoPayAmount" type="number" min="0.01" step="0.01"></div><div class="field"><label>Payment Date</label><input id="quoPayDate" type="date"></div><div class="field"><label>Method</label><select id="quoPayMethod"><option>Bank Transfer</option><option>Cash</option><option>Other</option></select></div><div class="field"><label>Reference</label><input id="quoPayReference" placeholder="Optional transfer / cash reference"></div></div>
  ...
  function openPaymentModal(invoiceId){
    ...
    document.getElementById('quoPayError').classList.add('hidden');document.getElementById('quoPaymentModal').classList.remove('hidden');
  ...
  async function submitPayment(){
    ...
    if(amount<=0)return paymentError('Enter an amount greater than 0.');if(amount>c.balance+EPS)return paymentError(`Payment cannot exceed the balance of ${money(c.balance,invoice.currency)}.`);if(!date)return paymentError('Select the payment date.');
