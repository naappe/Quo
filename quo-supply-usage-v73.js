/* Quo v75 - Simple Admin-only supply usage.
   Workflow: choose an existing quotation/invoice -> add vendor/supply lines -> save. */
(function(){
  const state={rows:[],vendors:[],loading:false,search:'',documentId:''};
  ...
  function optionList(){
    const rows=docs(),quotes=rows.filter(d=>d.document_type==='quotation'),invoices=rows.filter(d=>d.document_type==='invoice');
  ...
  function renderSupplyUsage(){
    ...
    setActiveNav();
    $('#view').innerHTML=`
      <div class="page-head quo-supply-page-head"><div><div class="eyebrow">WHITE SAFFRON · INTERNAL</div><h2>Supply Usage</h2><p>Select an existing document and record what supplies were used for it.</p></div></div>
      <section class="panel quo-simple-supply-entry">
        <div class="panel-head"><div><h3>Record Supply</h3><p>Use the existing Quotation or Invoice. Add as many supply lines as needed.</p></div><span class="badge final">ADMIN ONLY</span></div>
        <form id="quoSupplyUsageForm" class="quo-simple-supply-form">
          ...
          <div class="quo-supply-form-actions"><span>Saving does not change the selected Quotation or Invoice.</span><button class="btn primary" id="quoSupplySave" type="submit">Save Supply Usage</button></div>
        </form>
      </section>
  ...
  async function saveUsage(e){
    ...
    if(!documentId)return alert('Select the existing Quotation or Invoice first.');
