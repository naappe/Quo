/* Quo v44 - single preview authority.
   Replaces quo-preview.js, quo-preview-fix.js, quo-live-preview.js and quo-preview-responsive.js. */
(function(){
  ...
  function schedulePreview(delay=35){clearTimeout(previewTimer);previewTimer=setTimeout(refreshLivePreview,delay)}
  ...
  function fitShell(shell){
    ...
    const available=Math.max(1,shell.clientWidth),naturalW=page.offsetWidth||794,naturalH=page.offsetHeight||1123,scale=Math.min(1,available/naturalW);
    page.style.transform=`scale(${scale})`;page.style.transformOrigin='top left';shell.style.height=`${Math.ceil(naturalH*scale)}px`;
  }
  const resizeObserver=typeof ResizeObserver==='function'?new ResizeObserver(entries=>entries.forEach(entry=>fitShell(entry.target))):null;
  ...
  document.addEventListener('input',e=>{if(e.target.closest?.('[data-field],[data-item-field]'))schedulePreview()},true);
  document.addEventListener('change',e=>{if(e.target.closest?.('[data-field],[data-item-field]'))schedulePreview(90)},true);
  ...
  attachPreviewShells(document);enhanceFullPreview();schedulePreview(60);
})();
