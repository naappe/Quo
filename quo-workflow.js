/* Quo v44 - shared workflow presentation and navigation.
...
(function(){
  ...
  if(!document.getElementById('quoWorkflowV44Style')){
    const st=document.createElement('style');
    st.id='quoWorkflowV44Style';
    st.textContent=`
      ...
      .wf-stage{min-width:0;background:#fff;border:1px solid #e3e6e7;border-radius:9px;overflow:hidden}.wf-stage-head{width:100%;border:0;background:#fff;display:grid;grid-template-columns:34px 1fr 18px;gap:9px;align-items:center;text-align:left;padding:13px 14px 10px;color:#25292b}.wf-stage-head:hover{background:#fafbfb}
      ...
      @media(max-width:1250px){.wf-sequence{grid-template-columns:1fr 18px 1fr}.wf-queues{grid-template-columns:1fr}.wf-connector{font-size:15px}}
      @media(max-width:820px){.wf-sequence{display:block}.wf-stage{margin-bottom:8px}.wf-connector{display:none}.wf-queue-row{grid-template-columns:70px 1fr auto}.wf-queue-money{display:none}}
