/* Quo v21 - filling guide, logical section navigation, lightweight validation */
(function(){
  ...
  function assess(cards){
    const d=window.S?.current;if(!d)return [];
    const states=cards.map(()=>true);
    // Document: manual issue date is required for numbering and save.
    ...
    // Event / Service: if enabled, require service dates and pax.
    ...
    // Items: at least one usable item, qty and price non-negative.
    ...
    // Menu: only required if switched on; no timing requirement.
    const menuIndex=cards.findIndex(c=>/^Menu$/i.test(c.querySelector('h3')?.textContent||''));
    if(menuIndex>=0)states[menuIndex]=!d.include_menu||!!String(d.menu_text||'').trim();
    // Payment: if advance enabled, require advance percentage and due date.
  ...
  function addWarnings(cards){cards.forEach(c=>c.querySelector('.section-warning')?.remove());const d=window.S?.current;if(!d)return;
  ...
  function decorate(){
    const main=q('.editor-main');if(!main||!window.S?.current)return;
  ...
  let timer;const schedule=()=>{clearTimeout(timer);timer=setTimeout(decorate,70)};
